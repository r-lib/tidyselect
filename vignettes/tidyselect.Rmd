...
vignette](https://tidyselect.r-lib.org/articles/syntax.html). In this
vignette, we describe how to include tidyselect in your own packages.
...
- __Variadic dots__ as in `dplyr::select()`.

    ```{r, eval = FALSE}
    mtcars %>% dplyr::select(mpg, cyl)
    ```

- __Named arguments__ as in `tidyr::pivot_longer()`. In this case,
  multiple inputs can be provided inside `c()` or by using boolean
  operators:
...
take it as a named argument. For more advice about this, see the
[Making data with `...`](https://design.tidyverse.org/dots-data.html)
...
on your use case, it may be easier to wrap around `dplyr::select()`.
...
selected data, they return the __positions__ of selected elements
inside the input data. If you don't need the selected positions and
can afford the dependency, you may consider wrapping around dplyr
instead.
...
__defuse__ some or all of their arguments, and resume evaluation later
on. Crucially, evaluation can be resumed in a different context or
according to different rules, which is often how domain-specific
languages are created in R.
...
defused expression and some data. They return a vector of positions
...
the vector of positions reflect the new names.
...
3. Use the vector of positions returned by `eval_select()` to subset
...
As an example of how to use the vector of positions returned by
