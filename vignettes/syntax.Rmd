...
# FIXME: `eval_select()` and `eval_rename()` should be exported
eval_select <- tidyselect:::eval_select
eval_rename <- tidyselect:::eval_rename
...
- With normal vectors, the name is simply assigned to all selected
  inputs.
    ...
    ```{r}
    unclass(mtcars) %>% select_pos(foo = c(mpg, cyl))
    ```
...
unclass(mtcars) %>% select_pos(foo = mpg, foo = cyl)
