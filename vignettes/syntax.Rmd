...
of positions for the selected or renamed elements:
...
Another way of expressing set difference is binary `-`:
...
```{r}
mtcars %>% select_pos(foo = c(bar = mpg, baz = cyl))
```
...
- With data frames, a numeric suffix is appended because columns
  must be uniquely named.
    ...
    ```{r}
    mtcars %>% select_pos(foo = c(mpg, cyl))
    ```
...
- With normal vectors, the name is simply assigned to all selected
  inputs.
    ...
    ```{r}
    as.list(mtcars) %>% select_pos(foo = c(mpg, cyl))
    ```

Combination and propagation can be composed by using nested `c()`:

```{r}
mtcars %>% select_pos(foo = c(bar = c(mpg, cyl)))
```
...
to all elements of the data, and should return `TRUE` or `FALSE` to
...
attempts to find a predicate function in the environment. This
introduces an ambiguity when a predicate function has the same name as
a column. This is a conscious design decision:
...
columns, and force predicate functions with `!!` or `force()`.
`all_of()` is a way to explicitly opt for data lookup because it fails
if the data frame doesn't contain the columns, even if a function of
the same name is found in the environment:
...
```{r, error = TRUE}
mtcars %>% select_pos(mpg)

mtcars %>% rename_pos(mpg)
```
...
as.list(mtcars) %>% select_pos(foo = mpg, foo = cyl)

# Data frames cannot
mtcars %>% select_pos(foo = mpg, foo = cyl)
```
...
```{r, error = TRUE}
mtcars %>% select_pos(cyl, cyl = mpg)

mtcars %>% select_pos(disp, cyl = mpg)
```

This is not possible when renaming.

```{r, error = TRUE}
mtcars %>% rename_pos(cyl, cyl = mpg)

mtcars %>% rename_pos(disp, cyl = mpg)
```
...
```{r}
mtcars %>% select_pos(foo = cyl, cyl = mpg)

mtcars %>% rename_pos(foo = cyl, cyl = mpg)
```
...
The duplicate names can be repaired by renaming chosen positions:

```{r}
dups %>% select_pos(x, foo = 3)

dups %>% rename_pos(foo = 3)
```
...
names are bound to their positions in the data frame. This allows `:`
to create sequences of variable positions. The positions can be
